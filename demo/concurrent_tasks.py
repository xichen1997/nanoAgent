...
def main():
    ...
    client_a = SidecarClient(SIDECAR_A_URL)
    client_b = SidecarClient(SIDECAR_B_URL)

    try:
        # ── Setup: create trunk ────────────────────────────────────────────────
        header("SETUP: Create initial trunk")
        setup = client_a.start()
        info(f"Setup session: {setup['session_id']}")
        trunk_resp = client_a.trunk_init()
        ...
        client_a.execute_tool("bash_write", {"command": "echo 'task_a_complete' > /tmp/task_a.txt"})
        nota("A", "Wrote /tmp/task_a.txt")

        # ── Phase 1: Fork B ────────────────────────────────────────────────────
        header("PHASE 1B: Fork B from same trunk")
        fork_b = client_b.trunk_fork()
        ...
        client_b.execute_tool("bash_write", {"command": "echo 'task_b_complete' > /tmp/task_b.txt"})
        nota("B", "Wrote /tmp/task_b.txt")

        # ── Phase 2: Fork A commits first ─────────────────────────────────────
        header("PHASE 2: Fork A commits → new trunk")
        commit_a = client_a.trunk_commit()
        ...
        commit_b = client_b.trunk_commit()
        ...
        r = client_b.execute_tool("bash_write", {"command": "echo 'task_b_complete' > /tmp/task_b.txt"})
        nota("B2", f"Re-wrote /tmp/task_b.txt: {r['result']!r}")

        # Verify /tmp/task_a.txt is also there (inherited from trunk via A's commit)
        read_a = client_b.execute_tool("bash_read", {"command": "cat /tmp/task_a.txt"})
        ...
        commit_b2 = client_b.trunk_commit()
